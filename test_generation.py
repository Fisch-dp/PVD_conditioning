...
class GaussianDiffusion:
    ...
    def p_sample(self, denoise_fn, data, t, descriptions, noise_fn, clip_denoised=False, return_pred_xstart=False, use_var=True):
        """
    ...
    def reconstruct(self, x0, t, descriptions, denoise_fn, noise_fn=torch.randn, constrain_fn=lambda x, t:x):
...
class Model(nn.Module):
    def __init__(self, args, betas, loss_type: str, model_mean_type: str, model_var_type:str):
        ...
        self.model = PVCNN2(num_classes=args.nc, embed_dim=args.embed_dim, use_att=args.attention,
                            dropout=args.dropout, extra_feature_channels=771)
    ...
    def get_loss_iter(self, data, descriptions, noises=None):
        ...
        losses = self.diffusion.p_losses(
            denoise_fn=self._denoise, data_start=data, t=t, descriptions=descriptions, noise=noises)
        assert losses.shape == t.shape == torch.Size([B])
        return losses

    def gen_samples(self, shape, device, descriptions, noise_fn=torch.randn, constrain_fn=lambda x, t:x,
    ...
    def reconstruct(self, x0, t, descriptions, constrain_fn=lambda x, t:x):

        return self.diffusion.reconstruct(x0, t, descriptions, self._denoise, constrain_fn=constrain_fn)
...
def normalize(sample):
    # Reshape the point cloud to [3, 2048] for easier manipulation
    print('sample shape:', sample.shape)

   # Step 1: Center the point cloud
    centroid = torch.mean(sample, dim=2, keepdim=True)
    centered_points = sample - centroid

    # Step 2: Scale the point cloud
    max_distance = torch.max(torch.sqrt(torch.sum(centered_points ** 2, dim=1, keepdim=True)))
...
def generate(model, opt):
    ...
    with torch.no_grad():
        ...
        for i, data in tqdm(enumerate(test_dataloader), total=len(test_dataloader), desc='Generating Samples'):
            ...
            gen = model.gen_samples(x.shape,'cuda', description, clip_denoised=False).detach().cpu()
            ...
            save_point_cloud(gen, os.path.join(str(Path(opt.eval_path).parent), f"sample_{i}.ply"))
            save_point_cloud(x, os.path.join(str(Path(opt.eval_path).parent), f"ref_{i}.ply"))
            save_description(description, os.path.join(str(Path(opt.eval_path).parent), f"descr_{i}.txt"))
...
def save_description(descr, file):
    with open(file, 'w') as f:
        f.write(f"{descr}\n")
    print(f"Description saved to {filename}")
    
def save_point_cloud(point_cloud, filename):
    #assert point_cloud.shape == (1, 10000, 3), "Expected shape [1, 2048, 3]"    
