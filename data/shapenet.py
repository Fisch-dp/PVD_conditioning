...
from project.data.binvox_rw import read_as_3d_array
...
class ShapeNetVox(torch.utils.data.Dataset):
    ...
    dataset_path = Path("project/data/ShapeNetVox32")  # path to voxel data - make sure you've downloaded the ShapeNet voxel models to the correct path
    class_name_mapping = json.loads(Path("project/data/shape_info.json").read_text())  # mapping for ShapeNet ids -> names
    ...
    def __init__(self, split):
        ...
        self.items = Path(f"project/data/splits/shapenet/{split}.txt").read_text().splitlines()  # keep track of shapes based on split
    ...
    def __getitem__(self, index):
        ...
        item_class, item_identifier = item.split('/')
        #print('item', item)
...
class ShapeNetPoints(torch.utils.data.Dataset):
    num_classes = 13  # we'll be performing a 13 class classification problem
    dataset_path = Path("project/data/ShapeNetPointClouds/")  # path to point cloud data
    class_name_mapping = json.loads(Path("project/data/shape_info.json").read_text())  # mapping for ShapeNet ids -> names
    classes = sorted(class_name_mapping.keys())
    ...
    def get_point_cloud(shapenet_id):
        ...
        # TODO Imple
        obj_file = ShapeNetPoints.dataset_path / category_id / f'{shape_id}.obj'
        mesh = trimesh.load(obj_file, force='mesh')
        points = mesh.vertices[:1024]
        #if len(points) < 1024:
        #    points = np.pad(points, ((0, 1024 - len(points)), (0, 0)), mode="constant")
