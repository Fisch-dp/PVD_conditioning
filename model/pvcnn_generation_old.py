...
class PVCNN2Base(nn.Module):
    ...
    def forward(self, inputs, t, descriptions):
        ...
        ####Evtim - Process text descriptions
        #text_inputs = self.tokenizer(descriptions, return_tensors='pt', padding=True, truncation=True).to(inputs.device)
        #text_embeddings = self.text_encoder(**text_inputs).last_hidden_state[:, 0, :]  # Use [CLS] token
        #encoded_text = self.text_encoder(**self.tokenizer(descriptions, return_tensors="pt", padding=True, truncation=True).to(inputs.device))
        #text_features = self.text_fc(encoded_text.last_hidden_state.mean(dim=1))[:,:,None].expand(-1,-1,inputs.shape[-1])
        ####
        B, _, N= inputs.shape
        text_emb = self.text_embedding(descriptions, inputs.device).unsqueeze(-1).expand(-1, -1, inputs.shape[-1])
        ...
        for i, sa_blocks  in enumerate(self.sa_layers):
            #print(f"Before SA layer {i}, features shape: {features.shape}")
        ...
        #print(f"After SA layer {i}, features shape: {features.shape}")


                #features, coords, temb = sa_blocks ((torch.cat([features,temb, text_features],dim=1), coords, temb))###Evtim
        in_features_list[0] = inputs[:, 3:, :].contiguous()
        if self.global_att is not None:
            features = self.global_att(features)
        for fp_idx, fp_blocks  in enumerate(self.fp_layers):
            #print(f"Before FP layer {fp_idx}, features shape: {features.shape}")
            features, coords, temb = fp_blocks((coords_list[-1 - fp_idx], coords, torch.cat([features, temb], dim=1), in_features_list[-1 - fp_idx], temb))
            #print(f"After FP layer {fp_idx}, features shape: {features.shape}")

            #features, coords, temb = fp_blocks((coords_list[-1-fp_idx], coords, torch.cat([features,temb],dim=1), in_features_list[-1-fp_idx], temb))
            
           
        ###Evtim - Incorporate text embeddings into the final classifier
        #text_embeddings_expanded = text_embeddings[:, :, None].expand(-1, -1, features.shape[-1])
        #features = torch.cat([features, text_embeddings_expanded], dim=1)
        ####
