...
class GaussianDiffusion:
    ...
    def p_mean_variance(self, denoise_fn, data, t, descriptions, clip_denoised: bool, return_pred_xstart: bool):

        model_output = denoise_fn(data, t, descriptions)
    ...
    def p_sample(self, denoise_fn, data, t, descriptions, noise_fn, clip_denoised=False, return_pred_xstart=False):
        """
        Sample from the model
        """
        model_mean, _, model_log_variance, pred_xstart = self.p_mean_variance(denoise_fn, data=data, t=t, descriptions=descriptions, clip_denoised=clip_denoised,
    ...
    def p_sample_loop(self, denoise_fn, shape, device, descriptions,
        ...
        for t in reversed(range(0, self.num_timesteps if not keep_running else len(self.betas))):
            ...
            img_t = self.p_sample(denoise_fn=denoise_fn, data=img_t,t=t_, descriptions=descriptions, noise_fn=noise_fn,
                                  clip_denoised=clip_denoised, return_pred_xstart=False)

        assert img_t.shape == shape
        return img_t

    def p_sample_loop_trajectory(self, denoise_fn, shape, device, freq, descriptions,
                                 noise_fn=torch.randn,clip_denoised=True, keep_running=False):
        """
    ...
    def _vb_terms_bpd(self, denoise_fn, data_start, data_t, t, descriptions, clip_denoised: bool, return_pred_xstart: bool):
        true_mean, _, true_log_variance_clipped = self.q_posterior_mean_variance(x_start=data_start, x_t=data_t, t=t)
        model_mean, _, model_log_variance, pred_xstart = self.p_mean_variance(
            denoise_fn, data=data_t, t=t, descriptions=descriptions, clip_denoised=clip_denoised, return_pred_xstart=True)
    ...
    def p_losses(self, denoise_fn, data_start, t, descriptions, noise=None):
        """
        ...
        #print(noise.shape, data_start.shape)
        ...
        if self.loss_type == 'mse':
            ...
            eps_recon = denoise_fn(data_t, t, descriptions)
            ...
            losses = self._vb_terms_bpd(
                denoise_fn=denoise_fn, data_start=data_start, data_t=data_t, t=t, descriptions=descriptions, clip_denoised=False,
                return_pred_xstart=False)
        else:
            raise NotImplementedError(self.loss_type)
    ...
    def calc_bpd_loop(self, denoise_fn, x_start, descriptions, clip_denoised=True):
...
class PVCNN2(PVCNN2Base):
    ...
    #    ((128, 3, 16), (256, 0.2, 32, (128, 256))),
    #    ((256, 3, 8), (64, 0.4, 32, (256, 512))),
    #    (None, (16, 0.8, 32, (512, 512, 1024))),
    #]
    #fp_blocks = [
    #    ((512, 512), (256, 3, 8)),
    #    ((512, 512), (256, 3, 8)),
...
class Model(nn.Module):
    def __init__(self, args, betas, loss_type: str, model_mean_type: str, model_var_type:str):
        ...
        self.model = PVCNN2(num_classes=args.nc, embed_dim=args.embed_dim, use_att=args.attention,
                            dropout=args.dropout, extra_feature_channels=771)#0) 768 text emb + 3 position

    def prior_kl(self, x0):
        return self.diffusion._prior_bpd(x0)

    def all_kl(self, x0, descriptions, clip_denoised=True):
        total_bpd_b, vals_bt, prior_bpd_b, mse_bt =  self.diffusion.calc_bpd_loop(self._denoise, x0, descriptions, clip_denoised)
    ...
    ###Evtim
    def _denoise(self, data, t, descriptions):
        B, D,N= data.shape
        assert data.dtype == torch.float
        assert t.shape == torch.Size([B]) and t.dtype == torch.int64

        out = self.model(data, t, descriptions)

        assert out.shape == torch.Size([B, D, N])
        return out

    ##Evtim
    def get_loss_iter(self, data, descriptions, noises=None):
        B, D, N = data.shape
    ...
    def gen_samples(self, shape, device, descriptions,
                    noise_fn=torch.randn,
                    clip_denoised=True,
                    keep_running=False):
        descriptions = descriptions[:shape[0]] 
        return self.diffusion.p_sample_loop(self._denoise, shape=shape, device=device,
                                            descriptions=descriptions,
                                            noise_fn=noise_fn,
                                            clip_denoised=clip_denoised,
                                            keep_running=keep_running)

    def gen_sample_traj(self, shape, device, descriptions, freq, noise_fn=torch.randn,
...
def get_betas(schedule_type, b_start, b_end, time_num):
    if schedule_type == 'linear':
        betas = np.linspace(b_start, b_end, int(time_num))
...
def get_dataset(dataroot, npoints,category):
    tr_dataset = ShapeNet15kPointClouds(
        ...
        tr_sample_size=int(npoints),
        te_sample_size=int(npoints),
        scale=1.,
...
def train(gpu, opt, output_dir, noises_init):
    ...
    ''' data '''
    train_dataset, _ = get_dataset(opt.dataroot, int(opt.npoints), opt.category)
    ...
    for epoch in range(start_epoch, opt.niter):
        ...
        for i, data in enumerate(dataloader):
            ...
            if opt.distribution_type == 'multi' or (opt.distribution_type is None and gpu is not None):
                ...
                ###Evtim
                #descriptions = descriptions.cuda(gpu)
            elif opt.distribution_type == 'single':
                x = x.cuda()
            ...
            ##evtim
            loss = model.get_loss_iter(x, descriptions, noises_batch).mean()
        ...
        if (epoch + 1) % int(opt.diagIter) == 0 and should_diag:

            logger.info('Diagnosis:')

            x_range = [x.min().item(), x.max().item()]
            kl_stats = model.all_kl(x, descriptions)
            logger.info('      [{:>3d}/{:>3d}]    '
                         'x_range: [{:>10.4f}, {:>10.4f}],   '
                         'total_bpd_b: {:>10.4f},    '
                         'terms_bpd: {:>10.4f},  '
        ...
        if (epoch + 1) % int(opt.vizIter) == 0 and should_diag:
            logger.info('Generation: eval')

            model.eval()
            with torch.no_grad():
                predefined_descriptions = ["4 legs chair", "rounded chair legs", "chair without backrest"]
                   # Repeat or truncate descriptions to match batch size
                eval_batch_size = 25
                if len(predefined_descriptions) < eval_batch_size:
                    predefined_descriptions = (predefined_descriptions * (eval_batch_size // len(predefined_descriptions) + 1))[:eval_batch_size]
                elif len(predefined_descriptions) > eval_batch_size:
                    predefined_descriptions = predefined_descriptions[:eval_batch_size]
                    
                    
                x_gen_eval = model.gen_samples(new_x_chain(x, 25).shape, x.device,
                                               descriptions=predefined_descriptions, 
                                               clip_denoised=False)
                x_gen_list = model.gen_sample_traj(new_x_chain(x, 1).shape, x.device,
                                                   descriptions=predefined_descriptions,
                                                   freq=40, 
                                                   clip_denoised=False)
...
###Evtim
def generate_model_from_description(description, model, device):
    model.eval()
    tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')

    with torch.no_grad():
        encoded_text = model.model.text_encoder(**tokenizer(description, return_tensors="pt", padding=True, truncation=True).to(device))
        text_features = model.model.text_fc(encoded_text.last_hidden_state.mean(dim=1))[:, :, None]

        # Generate initial noise
        shape = (1, model.diffusion.num_timesteps, 2048)
        img_t = torch.randn(shape, dtype=torch.float, device=device)

        for t in reversed(range(0, model.diffusion.num_timesteps)):
            t_ = torch.empty(1, dtype=torch.int64, device=device).fill_(t)
            img_t = model.diffusion.p_sample(model._denoise, data=img_t, t=t_, noise_fn=torch.randn, clip_denoised=True)
...
def main():
    ...
    ''' workaround '''
    train_dataset, _ = get_dataset(opt.dataroot, int(opt.npoints), opt.category)
    noises_init = torch.randn(len(train_dataset), int(opt.npoints), opt.nc)
...
def parse_args():
    ...
    parser.add_argument('--dropout', default=0.0)#1)
